    ...
    def __ModifyQueueOptions(self):
        ...
        if choice == 'a':
            ...
            if self._CurrentPlayer.SameAs(self._Players[0]):
                self._Players[1].__Queue = self._CurrentPlayer.ReplaceQueue(self._Players[1].__Queue)
            else:
                self._Players[0].__Queue = self._CurrentPlayer.ReplaceQueue(self._Players[0].__Queue)
        elif choice == 'c':
            self._CurrentPlayer.SwapFirstAndLast()
        elif choice == 'd':
            self._CurrentPlayer.MoveItemToFront(int(input("Enter the index")))
        else:
            return False
        return True
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3), 6 to modify the queue or 9 to take the offer: "))
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
            if Choice == 6:
                if self.__ModifyQueueOptions() == False:
...
class MoveOptionQueue:
    ...
    def ReverseQueue(self):
        self.__Queue = self.__Queue.reverse()
