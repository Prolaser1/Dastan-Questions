...
class WeatherEvent:
    ...
    def SetWeatherLocation(self, SquareReference):
        col = int(SquareReference) % 10
        self.Location = col
...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            self.__DisplayState()
            if self.WeatherEventOccurs():
                event = WeatherEvent(2)
                event.SetActive(True)                
                squareList = []
                for row in range(1, self._NoOfRows +1):
                    for col in range(1, self._NoOfColumns +1):
                        if self._Board[self.__GetIndexOfSquare(int(str(row)+str(col)))].GetPieceInSquare() is None:
                              squareList.append(str(row)+str(col))
                WeatherSquare = squareList[random.randint(0, len(squareList)-1)]
                event.SetWeatherLocation(WeatherSquare)
        ...
        if event.IsActive():
            if not event.CountDownComplete():
                event.Countdown =- 1
            else:
                col = 3
                for row in range(1, self._NoOfRows+1):
                    if self._Board[self.__GetIndexOfSquare(int(str(row)+str(col)))].GetPieceInSquare() is not None:
                        self._Board[self.__GetIndexOfSquare(int(str(row)+str(col)))].RemovePiece()
