...
class Dastan:
    ...
    def __CheckSquareIsValid(self, SquareReference, StartSquare):
        #Modify To make sure if sqaure is barrier it cannot enter it
    ...
    #CheckBarrierIsValid
    def __CheckBarrierIsValid(self, BarrierReference):
        BarrierIsValid = True
        Row = BarrierReference // 10
        Col = BarrierReference % 10
        for col in range(Col-1, Col+2):
            if not self.__CheckSquareInBounds(Row*10+col):
                BarrierIsValid = False
            else:
                pieceL = self._Board[self.__GetIndexOfSquare(Row*10+col)]
                if pieceL.ContainsKotla() or pieceL.ContainsBarrier() or pieceL.GetPieceInSquare() != None:
                    BarrierIsValid = False
        return BarrierIsValid
        
    
    #PlaceBarrier 
    def PlaceBarrier(self, BarrierReference):
        Row = BarrierReference // 10
        Col = BarrierReference % 10
        if self.__CheckBarrierIsValid(BarrierReference):
            for col in range(Col-1, Col+2):
                if self._CurrentPlayer.SameAs(self._Players[0]):
                    self._Board[self.__GetIndexOfSquare(col*10+Row)].SetPiece(self._CurrentPlayer, "B")
                else:
                    self._Board[self.__GetIndexOfSquare(col*10+Row)].SetPiece(self._CurrentPlayer, "b")
            
    
    #CheckManhattenDistance
    def __CheckManhattanDistance(self):
